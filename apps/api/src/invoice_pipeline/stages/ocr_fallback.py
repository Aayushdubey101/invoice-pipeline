...
async def ocr_fallback(doc: Document) -> Document:
    try:
        ...
        log.error("pipeline_stage_error", stage="ocr_fallback", document_id=doc.document_id, error=str(exc))
...
def _load_engine(allow_paddle: bool = True):
...
async def _extract_with_fallback(doc: Document):
    engine = _load_engine()
    try:
        return await engine.extract_pages(doc.file_bytes, doc.mime_type)
    except (ImportError, ModuleNotFoundError):
        log.warning("ocr_engine_runtime_fallback", reason="paddleocr unavailable at runtime, using tesseract")
