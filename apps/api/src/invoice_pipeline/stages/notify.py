...
async def notify(doc: Document) -> Document:
    ...
    try:
        ...
        log.info("pipeline_stage", stage="notify", document_id=doc.document_id, status_code=resp.status_code)
    except Exception as exc:
        log.warning("pipeline_stage_warn", stage="notify", document_id=doc.document_id, error=str(exc))
